...
- Windows 10/11
- .NET Runtime 10系、または同梱/配布形態に合うruntime
- PowerShell
...
```powershell
.\workbox.exe run --name smoke --timeout-seconds 5 -- pwsh -NoProfile -Command "Start-Sleep -Seconds 2; 'done'"
.\workbox.exe list
.\workbox.exe prune
```
...
```powershell
.\workbox.exe run --name web -- cmd /c "npm run dev"
```
...
```powershell
.\workbox.exe list
.\workbox.exe inspect web
```

止める:

```powershell
.\workbox.exe stop web
```

## Next.jsが既に動いているか見る

```powershell
.\workbox.exe doctor nextjs --path C:\dev\example-next-app
```
...
```powershell
.\workbox.exe doctor nextjs --path <project-dir>
```
